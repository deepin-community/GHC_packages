...
${haskell:Depends} to the Depends: line, and the same with Recommends and
Suggests.

In rules, if you're using CDBS, include /usr/share/cdbs/1/rules/debhelper.mk
and /usr/share/cdbs/1/class/hlibrary.mk.  This is the current recommended way
to package a Haskell Library.

If you're using debhelper v7, use dh S<I<command>> --with haskell_devscripts .

If you're simply using debhelper, add dh_haskell in the install target and
dh_haskell_depends before dh_gencontrol in binary.  Your build and configure
targets should be empty.  Remember that you should add -a to all debhelper
calls for multi-binary packages.  That goes for dh_haskell too.  Your clean
