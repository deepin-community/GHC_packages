...
CABAL_PACKAGE ?= $(shell cat debian/control |\
 perl -ne 'print "$$1" if (/^Package:\s*libghc6-(.*)-dev\s*$$/i);')
...
DEB_DEV_PROVIDES_DOC = $(shell test \! -e debian/control || \
cat debian/control | perl -e 'while (<>) { \
    $$last_field=$$1 if m/^([-a-z0-9]*):/i; \
    $$is_dev=1 if $$last_field eq "Package" && m/:\s*libghc.*-dev/; \
    $$provides_doc=1 if $$last_field eq "Provides" && m/-doc\b/; \
    dotest();} dotest(); \
sub dotest { \
if (m/^\s*$$/) { \
    if ($$provides_doc && $$is_dev) {print "YES\n"; exit 0; \
    } else {$$provides_doc=0; $$is_dev=0;} \
}}')

DEB_SETUP_BIN_NAME ?= debian/hlibrary.setup
ifneq ($(DEB_DEV_PROVIDES_DOC),YES)
DEB_HADDOCK_HTML_DIR ?= /usr/share/doc/libghc6-$(CABAL_PACKAGE)-doc/html/
else
DEB_HADDOCK_HTML_DIR ?= /usr/share/doc/libghc6-$(CABAL_PACKAGE)-dev/html/
endif
